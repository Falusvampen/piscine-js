// Subject
// Create a function named format which accepts a valid Date and a format string. 
...
https: function format(date, formater) {
  const monthArray = [
    "January",
    "February",
    "March",
  ...
  for (let i = 0; i < formaterString.length; i++) {
    if (formaterString[i].match(/[A-Za-z ]/g)) {
      ...
      if (formaterString[i - 1] == " ") {
        formaterString =
          formaterString.slice(0, i) +
          " " +
          "µ" +
          formaterString[i] +
          " " +
          formaterString.slice(i + 1);
      } else {
        formaterString =
          formaterString.slice(0, i) +
          " " +
